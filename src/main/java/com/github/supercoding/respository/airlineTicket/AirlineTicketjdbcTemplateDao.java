...
@Repository
public class AirlineTicketjdbcTemplateDao implements AirlinTicketRepository {
    ...
    static RowMapper<AirlineTicket> airlineTicketRowMapper = ((rs, rowNums) -> new AirlineTicket(
            rs.getInt("ticket_id"),
    ...
    public List<AirlineTicket> findAllAirlineTicketsWithPlaceAndTicketType(String likePlace, String ticketType) {
        return jdbcTemplate.query("SELECT * FROM airline_ticket WHERE arrival_loc = ? AND ticket_type = ?",airlineTicketRowMapper,likePlace,ticketType);
    }
