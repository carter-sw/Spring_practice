...
public class ElectronicStroeController {
    ...
    public ElectronicStroeController(ElectronicStoreItemRepository electronicStoreItemRepository) {
    ...
    private List<Item> items = new ArrayList<>(Arrays.asList(
            ...
            new Item(String.valueOf(serialItemId++),"Microsoft Xbox Series X","Gaming Console",4990000,"Custom AMD Zen 2","1TB SSD")));
    ...
    public List<Item> findItemByQueryIds(@RequestParam("id") List<String> ids){
        ...
        List<Item> itemFound = items.stream()
                .filter( item -> idset.contains(item.getId())).collect(Collectors.toList());
